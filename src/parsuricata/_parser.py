...
grammar = r'''
    ...
    rule: action protocol target port direction target port _NEWLINE* "(" body ")"
    ...
    settings: "!" settings_item   -> negated_settings
            | settings_item ("," settings_item)*
    
    ?settings_item: STRING        -> string
                  | VALUE         -> literal
    
    // Robust quoted string (supports \\xNN, \\uNNNN, \\" etc.)
    STRING.4: /"([^"\\\r\n]|\\.)*"/
    VALUE.2: /([^,;\\]|\\.)+/
'''
